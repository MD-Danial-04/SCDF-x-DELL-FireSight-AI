...
final class RoomScannerViewController: UIViewController, RoomCaptureViewDelegate, RoomCaptureSessionDelegate {
    ...
    func captureView(shouldPresent roomDataForProcessing: CapturedRoomData, error: (any Error)?) -> Bool {
        ...
        // Let RoomCaptureView run its built-in processing and deliver the
        // result through didPresent. The processed CapturedRoom still carries
        // per-element confidence and object categories, so the fire-hazard
        // enrichment downstream is unaffected.
        //
        // NOTE: we deliberately do NOT run an offline RoomBuilder pass here.
        // Re-processing the whole scan at finish is heavy and, when it shares
        // the main thread with session teardown, can starve the UI long enough
        // for the watchdog to terminate the app.
        return true
    }
